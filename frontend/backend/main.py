...
# Load .env file if it exists (simple loader, no dotenv dependency)
...
async def stub():
    ...
    if _stub_html is None:
        stub_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "stub", "index.html")
...
async def parallax_generate(
    image: Annotated[UploadFile, File(description="Product photo")],
    mode: Annotated[str, Form()] = "3d",
):
    ...
    update_job(job.job_id, input_images=input_images, mode=mode)
...
async def parallax_get_job(job_id: str):
    ...
    # Build result — TwoDResult for 2d mode, ModelResult for 3d
    result = None
    error = None
    if px_status == "done" and job.mode == "2d":
        # 2D mode: return TwoDResult with explode video URL and/or frames
        explode_video = job.explode.get("video_url", "") if job.explode else ""
        explode_frames = job.explode.get("frames", []) if job.explode else []
        frame_count = len(explode_frames) if explode_frames else settings.frame_count

        # Only set video_url if there's a real video (not a data URI frame)
        video_url = explode_video if explode_video and not explode_video.startswith("data:") else ""

        # Get object info from manual_json
        manual = job.manual_json or {}
        obj_info = manual.get("object", {}) if isinstance(manual, dict) else {}
        ...
        result = {
            ...
            "explode_frames": explode_frames if explode_frames else None,
            "object_type": obj_info.get("type", ""),
            "likely_model": obj_info.get("likely_model", ""),
            "manual_url": manual_url,
            "pdf_url": pdf_url,
            "steps": manual.get("steps", []) if isinstance(manual, dict) else [],
            "object_summary": obj_info.get("summary", ""),
        }
    elif px_status == "done":
        parts = job.parts or []
        px_parts = []
